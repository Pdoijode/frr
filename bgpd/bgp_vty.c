						    ...
						    json_object *json)
{
	if (json) {
		...
		if (p->connection->t_gr_stale)
			json_object_int_add(
				json_timer, "gracefulStalepathTimerSec",
					...
					bool use_json, json_object *json_neigh)
{
	...
	if (use_json) {
		...
		if (p->connection->t_gr_restart)
			json_object_int_add(
				json_grace, "gracefulRestartTimerSec",
				event_timer_remain_second(p->connection->t_gr_restart));

		if (p->connection->t_gr_stale)
			json_object_int_add(
				json_grace, "gracefulStalepathTimerSec",
				event_timer_remain_second(p->connection->t_gr_stale));
				 ...
				 uint16_t sh_flags, bool use_json)
{
	...
	if (name) {
		if (strmatch(name, "all")) {
			bgp_show_all_instances_neighbors_vty(
				vty, type, ip_str, sh_flags, use_json);
			return CMD_SUCCESS;
		} else {
			bgp = bgp_lookup_by_name(name);
			if (!bgp) {
