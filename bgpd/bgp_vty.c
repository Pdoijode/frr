...
static int bgp_inst_gr_config_vty(struct vty *vty, struct bgp *bgp,
				  bool on)
{
...
static int bgp_global_gr_config_vty(struct vty *vty, bool on)
{
	struct listnode *node, *nnode;
	struct bgp *bgp;
	bool vrf_cfg = false;
	...
	if (vrf_cfg) {
		vty_out(vty, "%%Failed: global graceful-restart not permitted with per-vrf configuration\n");
		return CMD_WARNING;
	}
	...
	for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		ret = bgp_inst_gr_config_vty(vty, bgp, on);
		if (ret != BGP_GR_SUCCESS)
			vty_out(vty,
				"%% Applying global graceful-restart config to vrf %s failed, error %d\n",
				bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT ?
					"Default" : bgp->name, ret);
	}
      ...
      )
{
	if (vty->node == CONFIG_NODE)
		return bgp_global_gr_config_vty(vty, true);
	...
	ret = bgp_inst_gr_config_vty(vty, bgp, true);
...
{
	if (vty->node == CONFIG_NODE)
		return bgp_global_gr_config_vty(vty, false);
	...
	ret = bgp_inst_gr_config_vty(vty, bgp, false);
...
static int bgp_inst_gr_disable_config_vty(struct vty *vty, struct bgp *bgp,
					  bool on)
{
	int ret = BGP_GR_FAILURE;
...
static int bgp_global_gr_disable_config_vty(struct vty *vty, bool on)
{
	struct listnode *node, *nnode;
	struct bgp *bgp;
	bool vrf_cfg = false;
	...
	/* Initiate processing for all BGP instances. */
	for (ALL_LIST_ELEMENTS(bm->bgp, node, nnode, bgp)) {
		ret = bgp_inst_gr_disable_config_vty(vty, bgp, on);
		if (ret != BGP_GR_SUCCESS)
			vty_out(vty,
				"%% Applying global graceful-restart disable config to vrf %s failed, error %d\n",
				bgp->inst_type == BGP_INSTANCE_TYPE_DEFAULT ?
					"Default" : bgp->name, ret);
	}
...
{
	if (vty->node == CONFIG_NODE)
		return bgp_global_gr_disable_config_vty(vty, true);
	...
	ret = bgp_inst_gr_disable_config_vty(vty, bgp, true);
	if (ret == BGP_GR_SUCCESS) {
		vty_out(vty,
			"Graceful restart configuration changed, reset all peers to take effect\n");
...
{
	if (vty->node == CONFIG_NODE)
		return bgp_global_gr_disable_config_vty(vty, false);
	...
	ret = bgp_inst_gr_disable_config_vty(vty, bgp, false);
	if (ret == BGP_GR_SUCCESS) {
		vty_out(vty,
			"Graceful restart configuration changed, reset all peers to take effect\n");
	}
				 ...
				 json_object *json_neigh)
{
	...
	if (use_json)
		json_object_string_add(json_neigh, "bgpState",
				       lookup_msg(bgp_status_msg, p->status,
						  NULL));
	else
...
void bgp_vty_init(void)
{
	...
	install_element(CONFIG_NODE, &no_bgp_graceful_restart_stalepath_time_cmd);
	...
	install_element(CONFIG_NODE, &bgp_graceful_restart_select_defer_time_cmd);
	...
	install_element(CONFIG_NODE, &no_bgp_graceful_restart_rib_stale_time_cmd);
