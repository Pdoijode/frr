			  ...
			  bool use_json, json_object *json)
{
	...
	if (use_json) {
		...
		if (p->conf_if)
			json_object_string_addf(json_neigh, "neighborAddr",
						"%pSU", &p->connection->su);
		else {
			char neighborAddr[INET6_ADDRSTRLEN + 1] = { 0 };
			snprintf(neighborAddr, sizeof(neighborAddr), "%s%s",
				 dn_flag, p->host);

			json_object_string_add(json_neigh, "neighborAddr",
					       neighborAddr);
		}
	} else {
	...
	if (use_json) {
		...
		if ((p->as_type == AS_SPECIFIED) ||
		    (p->as_type == AS_EXTERNAL) || (p->as_type == AS_INTERNAL)) {
	...
	if (peer_established(p->connection) &&
		...
		if (use_json) {
			...
			if (CHECK_FLAG(p->cap, PEER_CAP_SOFT_VERSION_ADV)) {
				vty_out(vty, " advertised software version (%s)",
					cmd_software_version_get());
			} else
				vty_out(vty, " not advertised");
			...
			if (CHECK_FLAG(p->cap, PEER_CAP_RESTART_RCV) ||
				...
				if (CHECK_FLAG(p->cap, PEER_CAP_RESTART_RCV)) {
					...
					/* Graceful Restart */
					bgp_show_peer_gr_capability(vty, p,
								    use_json,
								    NULL);
				}
			}
