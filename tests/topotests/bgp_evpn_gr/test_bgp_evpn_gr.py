...
def fetch_l2vni_rd_from_pe2(pe2: TopoRouter, vni: int):
    """
...
def test_bgp_evpn_gr_stale_and_recovery():
    ...
    # Fetch VNI RD from PE2 for validation
    pe2_rd_vni_100 = fetch_l2vni_rd_from_pe2(pe2, 100)
    ...
    pe2_rd = "10.100.0.2:102"  # L3VNI RD of PE2 
    ...
    test_func = functools.partial(_evpn_routes_with_stale_only_for_rd, pe1, rd=pe2_rd, route_type=5)
    result, _ = topotest.run_and_expect(test_func, True, count=60, wait=2)
    assert result, (
...
def test_bgp_evpn_gr_stale_cleanup_on_timeout():
    ...
    test_func = functools.partial(_vrf_routes_absent, pe1, "vrf-blue", ["192.168.100.22"])
...
def test_bgp_evpn_gr_select_deferral_cleanup_on_pe2():
    ...
    test_func = functools.partial(_vrf_routes_absent, pe2, "vrf-blue", ["192.168.100.21"])
    result, _ = topotest.run_and_expect(test_func, True, count=160, wait=1)
